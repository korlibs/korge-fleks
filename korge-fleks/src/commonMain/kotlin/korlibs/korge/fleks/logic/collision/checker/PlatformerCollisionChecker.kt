...
class PlatformerCollisionChecker(
    ...
    override fun checkXCollision(
        ...
        val gridSize = worldMapData.tileSize
        var dir = 0
        val yrTop = yr + collisionBox.y / gridSize
        val yrBottom = yr + (collisionBox.y + collisionBox.height) / gridSize

        // Check direction of movement
        if (velocityX > 0f) {  // Moving right
            val xrRight = xr + (collisionBox.x + collisionBox.width) / gridSize
            ...
            repeat( ceil(collisionBox.height / gridSize).toInt()) { i ->
                ...
                if (worldMapData.hasCollision(cx + checkDistance, grid.cy + i) && xrRight >= checkRight) {  // Check the next X cell
                    dir = 1
                }
            }
            ...
            if (worldMapData.hasCollision(cx + checkDistance, grid.cy) && xrRight >= checkRight) {
                dir = 1
            }
        } else if (velocityX < 0f) {  // Moving left
            val xrLeft = xr + collisionBox.x / gridSize
            ...
            repeat( ceil(collisionBox.height / gridSize).toInt()) { i ->
                ...
                if (worldMapData.hasCollision(cx - checkDistance, grid.cy + i) && xrLeft <= checkLeft) {  // Check the next X cell
                    dir = -1
                }
            }
            // Cell coordinates of bottom corner of the collision box
            ...
            if (worldMapData.hasCollision(cx - checkDistance, grid.cy) && xrLeft <= checkLeft) {
                dir = -1
            }
        }
        return dir
    ...
    override fun checkYCollision(
        ...
        if (velocityY > 0f) {  // Moving down
            ...
            repeat( ceil(collisionBox.width / gridSize).toInt()) { i ->
                ...
                if (worldMapData.hasCollision(grid.cx + i, cy + checkDistance) && yrBottom >= checkBottom) {  // Check the next Y cell
                    dir = 1
                }
            }
            ...
            if (worldMapData.hasCollision(grid.cx, cy + checkDistance) && yrBottom >= checkBottom) {
                dir = 1
            }
            ...
            repeat( ceil(collisionBox.width / gridSize).toInt()) { i ->
                ...
                if (worldMapData.hasCollision(grid.cx + i, cy - checkDistance) && yrTop <= checkTop) {  // Check the next Y cell
                    dir = -1
                }
            }
            // Cell coordinates of right corner of the collision box
            grid.setAndNormalizeX(cx, xrRight)  // Right corner of the collision box
            debugShapesComponent?.let { debugSaveGridCell(debugShapesComponent, grid.cx, cy - checkDistance) }
            if (worldMapData.hasCollision(grid.cx, cy - checkDistance) && yrTop <= checkTop) {
                dir = -1
            }
        }
        return dir
