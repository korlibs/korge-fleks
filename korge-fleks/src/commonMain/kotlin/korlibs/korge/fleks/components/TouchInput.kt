...
class TouchInput private constructor(
    ...
    companion object {
        ...
        fun World.TouchInputComponent(config: TouchInput.() -> Unit ): TouchInput =
        getPoolable(TouchInputComponent).apply(config)

        // Call this function in the fleks world configuration to create the component pool
        fun InjectableConfiguration.addTouchInputComponentPool(preAllocate: Int = 0) {
            addPool(TouchInputComponent, preAllocate) { TouchInput() }
        }
    }

    // Clone a new instance of the component from the pool
    override fun World.clone(): TouchInput =
        getPoolable(TouchInputComponent).apply { init(from = this@TouchInput ) }
