...
class AssetStore {
    ...
    suspend fun watchAssetsForChanges(world: World, assetReloadContext: CoroutineContext, assetReloadCache: AssetReloadCache) {
        ...
        commonResourcesWatcher = resourcesVfs[commonAssetConfig.assetFolderName].watch {
            if (it.kind == Vfs.FileEvent.Kind.MODIFIED) { checkAssetFolders(world, it.file, AssetType.Common, commonAssetConfig, assetReloadContext, assetReloadCache) }
        }
        currentWorldResourcesWatcher = resourcesVfs[currentWorldAssetConfig.assetFolderName].watch {
            if (it.kind == Vfs.FileEvent.Kind.MODIFIED) { checkAssetFolders(world, it.file, AssetType.World, currentWorldAssetConfig, assetReloadContext, assetReloadCache) }
        }
        currentLevelResourcesWatcher = resourcesVfs[currentLevelAssetConfig.assetFolderName].watch {
            if (it.kind == Vfs.FileEvent.Kind.MODIFIED) { checkAssetFolders(world, it.file, AssetType.Level, currentLevelAssetConfig, assetReloadContext, assetReloadCache) }
        }
    }
