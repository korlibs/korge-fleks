...
Korge-fleks offers the following features for building a game with Kotlin:
...
- Generating of Asset clusters via running Gradle tasks to break assets down to be used per game progress (e.g. world chunk
  or level section)
- All loaded asset are in game available via a central asset store
...
- Automatic spawning and despawning of game objects depending on its position in relation to the camera view port
